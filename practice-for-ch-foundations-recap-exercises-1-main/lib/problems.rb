...
def all_vowel_pairs(words)
  ...
  words.each_with_index do |word1,idx1|
    words.each_with_index do |word2,idx2|
      pair = word1 + word2
      if idx1 < idx2 && (pair.include?("a") && pair.include?("e") && pair.include?("i") && pair.include?("o") && pair.include?("u"))
        pairs << "#{word1} #{word2}"
      end 
    end 
  end 
...
def find_bigrams(str, bigrams)
  newarr = []
  bigrams.each { |bigram| newarr << bigram if str.include?(bigram)}
  newarr
end
  ...
  def substrings(length = nil)
    ...
    self.each_char.with_index do |char1, idx1|
      self.each_char.with_index do |char2, idx2|
        subs << self[idx1..idx2] if idx1 <= idx2 
      end 
    end 
    if length != nil
