...
def bi_prime?(num)
    (2..num).each do |factor|
        if num % factor == 0
            factorpair = num / factor 
            if isprime?(factorpair) && isprime?(factor)
                return true
            end 
        end 
    end 
...
def vigenere_cipher(message, keys)
    ...
    message.each_char.with_index do |char, idx|
        ogidx = alphabet.index(char)
        num = keys[idx % length]
        newstr += alphabet[(ogidx + num) % 26]
    end 
...
def lastvowel(str)
    ...
    i = str.length - 1
    while i > 0 
        return str[i] if vowels.include?(str[i])
        i -= 1 
    end 
end 
